...
class SetFirebaseTokenInput {
  ...
  static SetFirebaseTokenInput? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'token'), 'Required key "SetFirebaseTokenInput[token]" is missing from JSON.');
        assert(json[r'token'] != null, 'Required key "SetFirebaseTokenInput[token]" has a null value in JSON.');
        return true;
      }());
